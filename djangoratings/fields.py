...
class RatingManager(object):
    def __init__(self, instance, field):
        ...
        self.votes_field_name = "{}_votes".format(self.field.name)
        self.score_field_name = "{}_score".format(self.field.name)
    ...
    def get_rating_for_user(self, user, ip_address=None, cookies={}):
        ...
        if use_cookies:
            # TODO: move 'vote-{}.{}.{}' to settings or something
            cookie_name = 'vote-{}.{}.{}'.format(
    ...
    def add(self, score, user, ip_address, cookies={}, commit=True):
        ...
        try:
            ...
            raise InvalidRating("{} is not a valid choice for {}".format(score, self.field.name))

        delete = (score == 0)
        if delete and not self.field.allow_delete:
            raise CannotDeleteVote("you are not allowed to delete votes for {}".format(self.field.name))
        ...
        use_cookies = (self.field.allow_anonymous and self.field.use_cookies)
        if use_cookies:
            defaults['cookie'] = now().strftime('%Y%m{}%H%M{}%f') # -> md5_hexdigest?
            # TODO: move 'vote-{}.{}.{}' to settings or something
            cookie_name = 'vote-{}.{}.{}' % (kwargs['content_type'].pk, kwargs['object_id'], kwargs['key'][:6],) # -> md5_hexdigest?
        ...
        try:
            ...
            if delete:
                raise CannotDeleteVote("attempt to find and delete your vote for {} is failed".format(self.field.name))
...
class RatingCreator(object):
    ...
    def __set__(self, instance, value):
        if isinstance(value, Rating):
            ...
            raise TypeError("{} value must be a Rating instance, not '{}'".format(self.field.name, value))
...
class RatingField(IntegerField):
    ...
    def __init__(self, *args, **kwargs):
        if 'choices' in kwargs:
            raise TypeError("{} invalid attribute 'choices'".format(self.__class__.__name__))
    ...
    def contribute_to_class(self, cls, name):
        ...
        cls.add_to_class("{}_votes".format(self.name), self.votes_field)

        # Score sum field
        self.score_field = IntegerField(
            editable=False, default=0, blank=True)
        cls.add_to_class("{}_score".format(self.name), self.score_field)
